...
class ExcelTableComparator:
    ...
    def _ensure_interface_no_as_string(self) -> None:
        """
        Ensure KKS values are properly formatted as strings.
        ...
        if 'KKS' in self.old_data.columns:
            self.old_data['KKS'] = self.old_data['KKS'].apply(format_interface_no)
        
        if 'KKS' in self.new_data.columns:
            self.new_data['KKS'] = self.new_data['KKS'].apply(format_interface_no)
        
        self.logger.info("KKS columns formatted as strings to preserve leading zeros")
    ...
    def validate_files_and_load_data(self) -> None:
        ...
        # Read Excel data with 'KKS' column as string to preserve leading zeros
        # Use dtype to ensure KKS is read as string
        try:
            self.old_data = pd.read_excel(self.old_file, sheet_name=old_sheet_name, dtype={'KKS': str})
            self.new_data = pd.read_excel(self.new_file, sheet_name=new_sheet_name, dtype={'KKS': str})
        except (KeyError, ValueError):
            # Fallback: read normally and then convert KKS column
        ...
        # Ensure KKS values are properly formatted as strings (preserve leading zeros)
    ...
    def _remove_added_deleted_rows_sections(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Remove rows from the bottom by finding the first row with an empty KKS
        and dropping that row and everything below it.
        """
        if len(df) == 0:
            return df
        
        # Check if 'KKS' column exists
        if 'KKS' not in df.columns:
            self.logger.warning("'KKS' column not found - skipping cleanup")
            return df
        
        # Find the first row index where 'KKS' is empty
        first_empty_idx = None
        for idx in range(len(df)):
            interface_no = df.iloc[idx]['KKS']
        ...
        if first_empty_idx is not None:
            ...
            self.logger.info(f"Found first empty KKS at row {first_empty_idx}, dropping {rows_dropped} rows")
    ...
    def _enrich_new_with_status_columns(self, old_df: pd.DataFrame, new_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        # Check if KKS column exists in both dataframes
        if 'KKS' not in old_df.columns or 'KKS' not in new_df.columns:
            self.logger.warning("KKS column missing - cannot enrich status columns")
            return new_df
        ...
        # Create lookup dictionary: KKS -> list of row indices in OLD
        old_interface_to_indices = defaultdict(list)
        for idx, interface_no in enumerate(old_df['KKS']):
        ...
        for idx, interface_no in enumerate(enriched_new['KKS']):
            if pd.notna(interface_no) and str(interface_no).strip() != '':
                new_interface_to_indices[interface_no].append(idx)
        
        # Process each unique KKS in NEW
        ...
        if duplicate_count > 0:
            self.logger.info(f"  {duplicate_count} rows involved duplicate KKS (1-to-1 matching)")
    ...
    def _write_data_rows(self, worksheet, result: ComparisonResult, formatting: dict) -> None:
        ...
        for row_idx in range(len(result.data)):
            ...
            for col_idx, col_name in enumerate(self.column_names, start=1):
                ...
                # Special handling for KKS to preserve leading zeros
                if col_name == 'KKS' and pd.notna(cell_value):
                    # Ensure KKS is written as text to preserve leading zeros
            ...
            # Mark duplicates with dark red font in KKS column
    ...
    def _write_added_rows_section(self, worksheet, result: ComparisonResult) -> None:
        ...
        for idx, row_data in enumerate(non_empty_added_rows):
            ...
            for col_idx, col_name in enumerate(self.column_names, start=1):
                ...
                # Special handling for KKS to preserve leading zeros
                if col_name == 'KKS' and pd.notna(value) and value != "":
...
def extract_sheets_and_compare(compare_file, output_file=None):
    ...
    try:
        ...
        # Find KKS column index for special handling
        ...
        for row_idx, row in enumerate(rows_to_copy[start_row:], start=1):
            for cell in row:
                ...
                # Special handling for KKS column to preserve leading zeros
        ...
        # Find KKS column index for special handling in new sheet
...
def main():
    ...
    default_input_file = r"C:\Users\szil\Repos\excel_wizadry\Line_List_Compare\compare.xlsx"
