...
'-----------------------------------------------------------
' Sub: ExecuteSimplifiedSearch
' Purpose: Execute search with simplified UI (From/To/CC fields + Search In checkboxes)
' Parameters:
'   criteria - Search criteria (FromAddress, ToAddress, CcAddress, SearchTerms, dates, folder)
'   searchSubject - Boolean: Search for SearchTerms in Subject
'   searchBody - Boolean: Search for SearchTerms in Body
' Notes: From/To/CC are always added if filled. Checkboxes control WHERE SearchTerms are searched.
'-----------------------------------------------------------
Public Sub ExecuteSimplifiedSearch(criteria As SearchCriteria, _
                                   searchSubject As Boolean, _
    ...
    searchTerms = Trim(criteria.SearchTerms)
    
    ' Add From/To/CC if provided (WHO criteria)
    If Len(Trim(criteria.FromAddress)) > 0 Then
        queryParts.Add "from:" & Trim(criteria.FromAddress)
    End If
    
    If Len(Trim(criteria.ToAddress)) > 0 Then
        queryParts.Add "to:" & Trim(criteria.ToAddress)
    End If
    
    If Len(Trim(criteria.CcAddress)) > 0 Then
        queryParts.Add "cc:" & Trim(criteria.CcAddress)
    End If
    
    ' Add Search Terms with WHERE checkboxes (subject/body)
    ' Need to apply field prefix to EACH term, not wrap the whole expression
    If Len(searchTerms) > 0 Then
        Dim whereParts As Collection
        Set whereParts = New Collection
        
        ' Build field-specific query for each checked location
        If searchSubject Then whereParts.Add ApplyFieldToTerms("subject", searchTerms)
    ...
    Else
        MsgBox "No search criteria specified.", vbExclamation, "No Search"
        Exit Sub
    End If
    
    ' Add date filters if provided
    If criteria.DateFrom > 0 And criteria.DateTo > 0 Then
        aqsQuery = aqsQuery & " AND received:" & FormatAQSDate(criteria.DateFrom) & ".." & FormatAQSDate(criteria.DateTo)
    ...
    If searchScope = olSearchScopeAllFolders Then
        Set searchExplorer = Application.Explorers.Add(targetFolder, olFolderDisplayNormal)
        searchExplorer.Display
        DoEvents
    Else
        Set searchExplorer = Application.ActiveExplorer
        If Not searchExplorer Is Nothing Then
            searchExplorer.CurrentFolder = targetFolder
            DoEvents
        Else
            Set searchExplorer = Application.Explorers.Add(targetFolder, olFolderDisplayNormal)
            searchExplorer.Display
    ...
    ' Debug output
    Debug.Print ""
    Debug.Print "========== SIMPLIFIED SEARCH =========="
    Debug.Print "Timestamp: " & Now
    Debug.Print "From: " & criteria.FromAddress
    Debug.Print "To: " & criteria.ToAddress
    Debug.Print "CC: " & criteria.CcAddress
    Debug.Print "Search Terms: " & searchTerms
    Debug.Print "Search In: Subject=" & searchSubject & " Body=" & searchBody
    Debug.Print "AQS Query: " & aqsQuery
    Debug.Print "Scope: " & GetScopeName(searchScope)
    Debug.Print "======================================="
    Debug.Print ""
...
ErrorHandler:
    Debug.Print "!!! SIMPLIFIED SEARCH ERROR !!!"
...
Public Sub ExecuteEmailSearch(criteria As SearchCriteria)
    ...
    ' Build AQS query from criteria FIRST (before touching Explorer)
    aqsQuery = BuildAQSQuery(criteria)
    
    ' Validate query is not empty BEFORE setting up Explorer
    ' This prevents permission errors when navigating with empty query
    If Len(Trim(aqsQuery)) = 0 Then
        Debug.Print "Empty query detected - no search criteria specified"
        MsgBox "No search criteria specified." & vbCrLf & vbCrLf & _
               "Please enter at least one search criterion and click Search again.", _
    ...
    ' Use active Explorer window instead of creating new one
    ' EXCEPT for All Folders scope - that requires a new window
    If searchScope = olSearchScopeAllFolders Then
        ' All Folders searches require a new Explorer window
        Debug.Print "All Folders scope - creating new Explorer window"
        Set searchExplorer = Application.Explorers.Add(targetFolder, olFolderDisplayNormal)
        searchExplorer.Display
        DoEvents
    Else
        ' For other scopes, try to use active Explorer
        Set searchExplorer = Application.ActiveExplorer
        
        ' Navigate to target folder in active Explorer
        If Not searchExplorer Is Nothing Then
            searchExplorer.CurrentFolder = targetFolder
            DoEvents  ' Allow UI to update
        Else
            ' Fallback: create new Explorer if none active
            Set searchExplorer = Application.Explorers.Add(targetFolder, olFolderDisplayNormal)
            searchExplorer.Display
            DoEvents
        End If
    ...
    ' Note: Query already validated as non-empty earlier
    searchExplorer.Search aqsQuery, searchScope
...
Private Function ResolveFolder(folderName As String) As Outlook.Folder
    ...
    ' Debug: See what we're actually looking for
    Debug.Print "ResolveFolder: Looking for '" & folderName & "' (uppercased: '" & trimmedName & "')"
    
    ' Handle empty or default
    If trimmedName = "" Or trimmedName = "---" Then
        Debug.Print "ResolveFolder: Empty or default, returning Inbox"
        Set folder = ns.GetDefaultFolder(olFolderInbox)
        Set ResolveFolder = folder
        Exit Function
    End If
    
    ' Handle "All Folders" special case - return Inbox as starting point
    ' (The search scope will handle searching across all folders)
    ' Use Contains check to be more robust against formatting variations
    If InStr(1, trimmedName, "ALL FOLDERS") > 0 Or _
       InStr(1, trimmedName, "EVERYWHERE") > 0 Or _
       trimmedName = "ALL FOLDERS" Or _
       trimmedName = "EVERYWHERE" Then
        Debug.Print "ResolveFolder: 'All Folders' detected, returning Inbox as starting point"
        Set folder = ns.GetDefaultFolder(olFolderInbox)
        Set ResolveFolder = folder
        Exit Function
    End If
...
Private Function DetermineSearchScope(criteria As SearchCriteria) As OlSearchScope
    ...
    ' Use Contains check for robustness
    If InStr(1, folderName, "ALL FOLDERS") > 0 Or InStr(1, folderName, "EVERYWHERE") > 0 Then
        Debug.Print "DetermineSearchScope: All Folders scope detected"
...
'-----------------------------------------------------------
' Function: ApplyFieldToTerms
' Purpose: Apply field prefix to each term in a search expression
' Parameters:
'   fieldName (String) - Field name (subject, body, etc.)
'   searchTerms (String) - Parsed search terms with AND/OR operators
' Returns: Field-prefixed query string
' Notes: Converts "wel* AND list" to "subject:wel* AND subject:list"
'        Converts "invoice OR receipt" to "subject:invoice OR subject:receipt"
'        Preserves operator precedence with parentheses for complex expressions
'-----------------------------------------------------------
Private Function ApplyFieldToTerms(fieldName As String, searchTerms As String) As String
    Dim result As String
    ...
    For i = LBound(tokens) To UBound(tokens)
        ...
        If Len(token) > 0 Then
            ...
            If UCase(token) = "AND" Or UCase(token) = "OR" Then
                ...
                result = result & " " & token & " "
            Else
                ' Apply field prefix to search term
